						...
						if (opcionModificar != 7) {
							...
							while (ingresoValido == false) {
								...
								try {
									...
									while (numeroHechizoModificar <= 0 || numeroHechizoModificar >= listadeHechizos.size()+ 1) {
										System.out.println("-----ERROR: NUMERO INVALIDO-----");
										System.out.println("Ingrese numero del Hechizo a Modificar");
										numeroHechizoModificar = Integer.parseInt(lector.nextLine());
									}
							...
							if (opcionModificar == 1) {
								...
								System.out.println("Ingrese Nuevo nombre para el hechizo: "+nombreHechizoModificar);
								...
								for (Hechicero hechicero : listaHechiceros) {
									if (hechicero.getHechizos().contains(hechizoModificar)) {
										...
										hechicero.getHechizos().get(posicion).setNombreHechizo(nuevoNombreHechizoModificar);										
									}	
								}
								listadeHechizos.get(numeroHechizoModificar-1).setNombreHechizo(nuevoNombreHechizoModificar);
								actualizarArchivoHechizo();
								actualizarArchivoMagos();
							}
							...
							if (opcionModificar == 2) {
								...
								System.out.println("Ingrese Nuevo Daño para el hechizo: "+nombreHechizoModificar);
							...
							if (opcionModificar == 3) {
								...
								actualizarArchivoHechizo();
								actualizarArchivoMagos();
								System.out.println("Se ha Ajustado con exito el hechizo: " +nombreHechizoModificar + " su Duracion de Quemadura ahora es de: " + duracionQuemadura+"(s)");
								}
									...
									actualizarArchivoHechizo();
									actualizarArchivoMagos();
									System.out.println("Se ha Ajustado con exito el hechizo: " +nombreHechizoModificar + " su Cantidad de Curacion ahora es de: " + cantCuracion + ", y su Presion ahora es de:"+presionAgua);
									...
									actualizarArchivoHechizo();
									actualizarArchivoMagos();
									System.out.println("Se ha Ajustado con exito el hechizo: " +nombreHechizoModificar + " su Mejora de Defensa ahora es de: " + mejoraDefensa);
									...
									actualizarArchivoHechizo();
									actualizarArchivoMagos();
									System.out.println("Se ha Ajustado con exito el hechizo: " +nombreHechizoModificar + " su Cantidad de Stun ahora es de: " + cantStun + ", y su Cantidad de plantas ahora es de:"+cantPlantas);

							}
							}
							if (opcionModificar == 4) {
								//Aplicar Mutacion Elemental, cambiar tipo del elemento y agregar Mutacion Elemental (nombreAntiguo) 
								System.out.println("---BIENVENIDO A LA MUTACION ELEMENTAL---");
								String tipoElemento = hechizoModificar.getTipo();
								System.out.println("A que elemento quieres mutar tu hechizo: " + nombreHechizoModificar + " (Actual Elemento: "+tipoElemento+")");
								...
								if (hechizoModificar instanceof Fuego) {
									...
									if (opcionMutacion == 1) {
										...
										String nombreMutacion = "Mutacion elemental de " +hechizoModificar.getNombreHechizo(); 
										int mejoraDefensa = obtenerDefensaNueva(nombreHechizoModificar);
										Tierra mutacionTierra = new Tierra(nombreMutacion, "Tierra", dañoHechizoModificar, mejoraDefensa);

										aplicarMutacion(hechizoModificar, mutacionTierra);
										System.out.println("Se ha aplicando con Exito la: " + nombreMutacion + " su Mejora de Defensa ahora es de: " + mejoraDefensa);
									}
										...
										int cantCuracion = datosAgua[0];
										int presionAgua = datosAgua[1];
										Agua mutacionAgua = new Agua(nombreMutacion, "Agua", dañoHechizoModificar, cantCuracion,presionAgua);

										aplicarMutacion(hechizoModificar, mutacionAgua);
										System.out.println("Se ha aplicando con Exito la: " + nombreMutacion + " su Cantidad de curacion ahora es de: " + cantCuracion+" y su presion de agua ahora es de:  "+ presionAgua);
									}
										...
										int cantStun = datosPlanta[0];
										int cantPlantas = datosPlanta[1];
										Planta mutacionPlanta = new Planta(nombreMutacion, "Planta", dañoHechizoModificar, cantStun,cantPlantas);
							
										aplicarMutacion(hechizoModificar, mutacionPlanta);
										System.out.println("Se ha aplicando con Exito la: " + nombreMutacion + " su tiempo de stun ahora es de: " + cantStun+" y su cantidad de plantas ahora es de:  "+ cantPlantas);
...
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
								...
								if (hechizoModificar instanceof Agua) {
									...
									if (opcionMutacion == 2) {
										...
										Fuego mutacionFuego = new Fuego(nombreMutacion, "Fuego", dañoHechizoModificar, duracionQuemadura);

										aplicarMutacion(hechizoModificar, mutacionFuego);
										System.out.println("Se ha aplicando con Exito la: " + nombreMutacion + " su tiempo de Quemadura ahora es de: " + duracionQuemadura);
...
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
							...
							if (opcionModificar == 5) {
								...
								while (ingresoValido == false) {
									...
									try {
										System.out.println("Ingrese Nuevo % de Daño para el hechizo: "+nombreHechizoModificar);
										System.out.println("Para aplicar un buff de daño el porcentaje debe ser mayor a 0 y menor a una mejora mayor del 200%");
										System.out.println("El parametro solo devolvera valores sin decimales redondeados (ejemplo: 104,45 --- 104 ).");	
										buffDañoHechizo = Integer.parseInt(lector.nextLine());
										
										while (buffDañoHechizo <= 0 || buffDañoHechizo >= 201) {
											System.out.println("-----ERROR: NUMERO INVALIDO-----");
											System.out.println("Para aplicar un buff de daño el porcentaje debe ser mayor a 0 y menor a una mejora mayor del 200%");
											System.out.println("El parametro solo recibe valores sin decimales (ejemplo: 15 %).");
											System.out.println("El parametro solo devolvera valores sin decimales redondeados (ejemplo: 104,45 --- 104 ).");	
											System.out.println("Ingrese Nuevo % de Daño para el hechizo: "+nombreHechizoModificar);
											buffDañoHechizo = Integer.parseInt(lector.nextLine());
										}
								...
								actualizarArchivoHechizo();
								actualizarArchivoMagos();
								System.out.println("Se ha buffeado con exito el hechizo: " +nombreHechizoModificar + " su daño ahora es de: " +dañoHechizoModificar);
							...
							if (opcionModificar == 6) {
								...
								while (ingresoValido == false) {
									...
									try {
										System.out.println("Ingrese Nuevo nerfeo del % de Daño para el hechizo: "+nombreHechizoModificar);
										System.out.println("Para aplicar un nerf de daño el porcentaje debe ser mayor a 0 y menor a un valor mayor del 99%");
										System.out.println("El parametro solo devolvera valores sin decimales redondeados (ejemplo: 104,45 --- 104 ).");	
								...
								actualizarArchivoHechizo();
								actualizarArchivoMagos();
								System.out.println("Se ha nerfeado con exito el hechizo: " +nombreHechizoModificar + " su daño ahora es de: " +dañoHechizoModificar);
...
public int[] obtenerDatosAguaNuevos(String nombreHechizoModificar) {
	...
	while (ingresoValido == false) {
		try {
			System.out.println("Ingresar Nueva Cantidad de Curacion del Hechizo: "+ nombreHechizoModificar + "(Curacion Anterior: "+ cantCuracion +")" );
			System.out.println("---RECORDATORIO---");
			System.out.println("Para que el juego tenga un equilibrio el hechizo no puede tener más de 100 de Curacion ni puede tener menos de 1 de Curacion.");	
			System.out.println("El parametro solo recibe valores sin decimales (ejemplo: 10).");	
			cantCuracion = Integer.parseInt(lector.nextLine());
...
public void aplicarMutacion(Hechizo hechizoModificar, Hechizo mutacion) {
	...
	else {System.out.println("ERROR, POSICION INEXISTENTE"); }
